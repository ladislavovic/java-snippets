...
public class TestCollectionIndexing extends HibernateSearchTest {
	...
	public void test() {
		...
		jpaService().doInTransactionAndFreshEM(em -> {
			...
			QueryDescriptor queryDescriptor = ElasticsearchQueries.fromQueryString("carModels:Mondeo");
			FullTextQuery query = fullTextEm.createFullTextQuery(queryDescriptor, Person.class);
			query.setProjection("name", ElasticsearchProjectionConstants.SOURCE); // NOTE: it is not possible to project "carModels". HS 5 can not project collections.
